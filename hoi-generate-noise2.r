...
NoHOIsamplesNoise2 <- function(globPars, SD) {
...
for (bpars in 1:nrow(listpars)) {
  pb$tick()
  x10 <- listpars$X10[bpars]
  x20 <- listpars$X20[bpars]
  x30 <- listpars$X30[bpars]
  ...
  tryCatch(
    {
      ...
      #dataOut2[index, 1:7] <- out2$x1
      #dataOut2[index, 8:14] <- out2$x2
      #dataOut2[index, 15:21] <- out2$x3
